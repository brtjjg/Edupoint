...
import json, os, sqlite3, logging, secrets, time
...
# ============================================================
# MULTI-NETWORK MOBILE MONEY PAYMENT SYSTEM
# Networks enabled here: Airtel Money + T-Kash
# M-PESA is intentionally NOT used by this implementation.
# ============================================================
PAYMENT_DB = os.environ.get("PAYMENT_DB", "payments.db")
PAYMENT_AMOUNT = 100
...
# T-Kash API details are supplied by Telkom when your merchant/biller API
# access is provisioned. Keep the endpoint configurable rather than inventing
# an endpoint or credentials that Telkom has not issued to you.
TKASH_CONFIG = {
    "base_url": os.environ.get("TKASH_BASE_URL", ""),
    "api_key": os.environ.get("TKASH_API_KEY", ""),
    "merchant_id": os.environ.get("TKASH_MERCHANT_ID", ""),
    "payment_path": os.environ.get("TKASH_PAYMENT_PATH", "/payments"),
    "timeout": 30,
}
...
def normalize_ke_phone(phone):
    """Return Kenyan MSISDN as 9 digits, e.g. 712345678."""
    ...
    if phone.startswith("+254"):
        ...
        phone = phone[1:]
    if len(phone) != 9 or not phone.isdigit() or phone[0] not in "17":
        raise ValueError("Enter a valid Kenyan mobile number.")
    return phone


def create_payment(phone, network, amount, plan):
    reference = "EDU-" + datetime.utcnow().strftime("%Y%m%d%H%M%S") + "-" + secrets.token_hex(4).upper()
    now = datetime.utcnow().isoformat() + "Z"
    conn = db_connect()
    conn.execute(
        "INSERT INTO payments(reference, phone, network, amount, plan, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 'PENDING', ?, ?)",
        (reference, phone, network, amount, plan, now, now),
    )
    conn.commit()
    conn.close()
    return reference
...
def update_payment(reference, status=None, provider_transaction_id=None,
                   provider_receipt=None, provider_message=None, raw_response=None):
    conn = db_connect()
    ...
    if provider_transaction_id is not None:
        fields.append("provider_transaction_id=?"); values.append(provider_transaction_id)
    if provider_receipt is not None:
        fields.append("provider_receipt=?"); values.append(provider_receipt)
    if provider_message is not None:
        fields.append("provider_message=?"); values.append(provider_message)
    if raw_response is not None:
        fields.append("raw_response=?"); values.append(json.dumps(raw_response)[:10000])
    fields.append("updated_at=?"); values.append(datetime.utcnow().isoformat() + "Z")
    values.append(reference)
    conn.execute(f"UPDATE payments SET {', '.join(fields)} WHERE reference=?", values)
    conn.commit()
    conn.close()
    return True
...
def airtel_base_url():
    return "https://openapi.airtel.africa" if AIRTEL_CONFIG["environment"] == "production" else "https://openapiuat.airtel.africa"


def get_airtel_token():
    if not AIRTEL_CONFIG["client_id"] or not AIRTEL_CONFIG["client_secret"]:
        raise RuntimeError("Airtel Money credentials are not configured.")
    r = requests.post(
        ...
        timeout=AIRTEL_CONFIG["timeout"],
    )
    r.raise_for_status()
    data = r.json()
    token = data.get("access_token")
    if not token:
        raise RuntimeError("Airtel did not return an access token.")
    return token


def initiate_airtel(reference, phone, amount):
    ...
    data = r.json()
    if not r.ok:
        raise RuntimeError(data.get("message") or data.get("status", {}).get("message") or "Airtel payment request failed.")
    tx = data.get("data", {}).get("transaction", {})
    return {
        "provider_transaction_id": tx.get("id") or reference,
        "status": str(tx.get("status") or "PENDING").upper(),
        "message": data.get("status", {}).get("message") or tx.get("message") or "Payment request sent.",
        "raw": data,
    }


def initiate_tkash(reference, phone, amount):
    """Call your Telkom-provided T-Kash merchant/biller endpoint.

    Telkom's public site documents T-Kash merchant/till/paybill services, but
    does not publish a stable public REST collection schema. Therefore this
    adapter intentionally uses credentials + endpoint supplied to your
    business by Telkom instead of inventing an API contract.
    """
    base_url = TKASH_CONFIG["base_url"].rstrip("/")
    if not base_url or not TKASH_CONFIG["api_key"]:
        raise RuntimeError("T-Kash API credentials/endpoint are not configured.")
    payload = {
        "reference": reference,
        "phone": "254" + phone,
        "amount": amount,
        "currency": "KES",
        "merchant_id": TKASH_CONFIG["merchant_id"],
        "callback_url": os.environ.get("PUBLIC_BASE_URL", "").rstrip("/") + "/api/payment/webhook/tkash",
    }
    r = requests.post(
        base_url + TKASH_CONFIG["payment_path"],
        json=payload,
        headers={
            "Content-Type": "application/json",
            "Authorization": f"Bearer {TKASH_CONFIG['api_key']}",
        },
        timeout=TKASH_CONFIG["timeout"],
    )
    try:
        data = r.json()
    except ValueError:
        data = {"text": r.text}
    if not r.ok:
        raise RuntimeError(data.get("message") or data.get("error") or "T-Kash payment request failed.")
    tx = data.get("transaction", data.get("data", {})) or {}
    return {
        "provider_transaction_id": tx.get("id") or tx.get("transaction_id") or reference,
        "status": str(tx.get("status") or "PENDING").upper(),
        "message": data.get("message") or "T-Kash payment request sent.",
        "raw": data,
    }
...
def payment_initiate():
    ...
    network = str(data.get("network", "")).lower().strip()
    plan = str(data.get("plan", "premium")).lower().strip()
    try:
        phone = normalize_ke_phone(data.get("phone"))
    except ValueError as e:
        return jsonify({"ok": False, "error": str(e)}), 400

    if network not in {"airtel", "tkash"}:
        return jsonify({"ok": False, "error": "Select Airtel Money or T-Kash."}), 400
    if plan not in {"basic", "premium", "pro"}:
        return jsonify({"ok": False, "error": "Invalid payment plan."}), 400

    amount = PAYMENT_AMOUNT
    reference = create_payment(phone, network, amount, plan)
    try:
        if network == "airtel":
            result = initiate_airtel(reference, phone, amount)
        else:
            result = initiate_tkash(reference, phone, amount)
        update_payment(
            reference,
            status=result["status"] if result["status"] in {"SUCCESS", "FAILED", "PENDING"} else "PENDING",
        ...
        return jsonify({
            ...
            "reference": reference,
            "network": network,
            "status": get_payment(reference)["status"],
            "message": result["message"],
        })
    except Exception as e:
        logger.exception("Payment initiation failed")
        update_payment(reference, status="FAILED", provider_message=str(e))
        return jsonify({"ok": False, "reference": reference, "error": str(e)}), 502
...
@app.route('/api/payment/webhook/airtel', methods=['POST'])
def airtel_webhook():
    data = request.get_json(silent=True) or {}
    tx = data.get("transaction") or data.get("data", {}).get("transaction", {}) or {}
    reference = tx.get("id") or data.get("reference")
    if not reference:
        return jsonify({"ok": False, "error": "Missing transaction reference."}), 400
    status = str(tx.get("status") or tx.get("status_code") or "PENDING").upper()
    if status in {"TS", "SUCCESS", "COMPLETED"}:
        status = "SUCCESS"
    elif status in {"TF", "FAILED", "FAILURE", "CANCELLED"}:
        status = "FAILED"
    else:
        status = "PENDING"
    update_payment(reference, status=status, provider_transaction_id=tx.get("airtel_money_id") or tx.get("id"), provider_message=tx.get("message"), raw_response=data)
    return jsonify({"ok": True}), 200


@app.route('/api/payment/webhook/tkash', methods=['POST'])
def tkash_webhook():
    data = request.get_json(silent=True) or {}
    tx = data.get("transaction") or data.get("data", {}).get("transaction", {}) or data
    reference = tx.get("reference") or tx.get("id") or data.get("reference")
    if not reference:
        return jsonify({"ok": False, "error": "Missing transaction reference."}), 400
    status = str(tx.get("status") or data.get("status") or "PENDING").upper()
    if status in {"SUCCESS", "COMPLETED", "PAID"}:
        status = "SUCCESS"
    elif status in {"FAILED", "FAILURE", "CANCELLED", "REVERSED"}:
        status = "FAILED"
    else:
        status = "PENDING"
    update_payment(reference, status=status, provider_transaction_id=tx.get("transaction_id") or tx.get("id"), provider_receipt=tx.get("receipt") or tx.get("receipt_number"), provider_message=tx.get("message"), raw_response=data)
    return jsonify({"ok": True}), 200
...
.bot-message {
    ...
    <body>
        <div class="container">
            ...
            <p>Access to premium features requires a one-time payment of KES 100 via an approved mobile-money network (Airtel Money or T-Kash). All payments are non-refundable unless technical issues prevent access. In case of errors, contact us at <a href="mailto:academichelpdesk1@gmail.com">academichelpdesk1@gmail.com</a>.</p>
...
def privacy():
    ...
    <body>
        <div class="container">
            ...
            <p>We collect the KCSE subjects and grades you enter, your mobile-money phone number (only for payment processing), and any information you voluntarily provide (e.g., email, interests). We also collect anonymous usage data via Google Analytics.</p>
            
            <h2>2. How We Use Your Information</h2>
            <p>Your grades are used solely to calculate cluster points and recommend courses. Phone numbers are used only to send mobile-money payment notifications (if you choose to pay). We do not sell or share your personal data with third parties.</p>
            
            <h2>3. Data Storage & Security</h2>
            <p>We do not permanently store your grades or results unless you create an account (future feature). Mobile-money transactions are processed through the selected payment provider; we do not store your mobile-money PIN. We take reasonable measures to protect your data.</p>
...
<div class="container">
    ...
    <p>Users must use only approved payment methods (Airtel Money or T-Kash), not send fake transaction messages or screenshots, not attempt chargeback fraud or manipulation, and ensure correct payment details. EduPoint may delay or verify payments before granting access.</p>
...
<div id="paymentPage" style="display:none; min-height:100vh; align-items:center; justify-content:center; padding:20px;">
    <div style="background:var(--card); border-radius:22px; padding:30px; max-width:450px; width:100%; text-align:center;">
        ...
        <div class="pay-box">
            ...
            <p style="font-size:.85em;color:var(--text2);">Choose your mobile-money network below.</p>
        </div>
        <select id="payNetwork" style="width:100%; margin:10px 0;">
            <option value="airtel">Airtel Money</option>
            <option value="tkash">T-Kash</option>
        </select>
        <input type="tel" id="payPhone" placeholder="07XX XXX XXX" style="width:100%; margin:10px 0;">
        <select id="payPlan" style="width:100%; margin:10px 0;">
            <option value="premium">Premium Plan — KES 100</option>
            <option value="basic">Basic Plan — KES 100</option>
            <option value="pro">Pro Plan — KES 100</option>
        </select>
        ...
        <button class="btn btn-calc" onclick="processPayment()">📱 Pay Now</button>
...
function processPayment() {
    ...
    var network = document.getElementById('payNetwork').value;
    var plan = document.getElementById('payPlan').value;
    var agreeCheckbox = document.getElementById('agreeTerms');

    if (!phone) { notify("Enter your mobile-money phone number", "error"); return; }
    if (!agreeCheckbox || !agreeCheckbox.checked) { notify("Agree to Terms & Privacy", "error"); return; }

    var statusBox = document.getElementById('payStatus');
    statusBox.innerHTML = '⏳ Starting ' + (network === 'airtel' ? 'Airtel Money' : 'T-Kash') + ' payment...';

    fetch('/api/payment/initiate', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({phone: phone, network: network, plan: plan})
    })
    .then(function(res) { return res.json().then(function(data) { return {ok: res.ok, data: data}; }); })
    .then(function(result) {
        var data = result.data;
        if (!result.ok || !data.ok) {
            statusBox.innerHTML = '<span style="color:var(--red);">❌ ' + (data.error || 'Payment could not be started.') + '</span>';
            return;
        }

        statusBox.innerHTML = '📲 ' + (data.message || 'Payment request sent. Approve it on your phone.') +
            '<br><small>Reference: ' + data.reference + '</small>';
        pollPaymentStatus(data.reference);
    })
    .catch(function(err) {
        console.error(err);
        statusBox.innerHTML = '<span style="color:var(--red);">❌ Network error. Please try again.</span>';
    });
}
...
function pollPaymentStatus(reference) {
    ...
    var timer = setInterval(function() {
        attempts++;
        fetch('/api/payment/status/' + encodeURIComponent(reference))
            ...
            .then(function(data) {
                ...
                if (p.status === 'SUCCESS') {
                    clearInterval(timer);
                    document.getElementById('payStatus').innerHTML = '<span style="color:var(--green);">✅ Payment confirmed! Redirecting...</span>';
                    setTimeout(function() {
                        document.getElementById('paymentPage').style.display = 'none';
                        document.getElementById('app').style.display = 'block';
                        if (typeof build === 'function') build();
                        else if (typeof buildPreview === 'function') buildPreview();
                        if (typeof notify === 'function') notify('✅ Payment confirmed. Premium access unlocked!', 'success');
                    }, 800);
                } else if (p.status === 'FAILED') {
                    clearInterval(timer);
                    document.getElementById('payStatus').innerHTML = '<span style="color:var(--red);">❌ Payment failed or was cancelled. Please try again.</span>';
                }
            })
            .catch(function(err) { console.error('Status check:', err); });

        if (attempts >= 36) {
            clearInterval(timer);
            document.getElementById('payStatus').innerHTML += '<br><span style="color:var(--yellow);">⏱️ Payment is still pending. Keep the reference and check again later.</span>';
        }
    }, 5000);
}
